...
def dice_loss(y_true: tf.Tensor, y_pred: tf.Tensor, smooth: float = 1e-6) -> tf.Tensor:
    """Multi-class Dice loss computed per channel and averaged."""
...
class CombinedSegmentationLoss(keras.losses.Loss):
    """Weighted combination: 0.5 Dice + 0.3 Focal + 0.2 Boundary."""
    ...
    def call(self, y_true, y_pred):
        ...
        b_loss = boundary_loss(y_true, y_pred, num_classes=self.config.num_classes)
        return (
